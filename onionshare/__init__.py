...
def main(cwd=None):
    ...
    parser.add_argument(
        ...
        default=0,
        dest="other_service",
        help="Publish a hiden service bound to the given port where your own server is running locally."
    )
    parser.add_argument(
        "--persist",
        action="store_true",
        dest="persist",
        help="Save the private key of the onionservice in the JSON config. You will be able to use the same address after a reboot"
    )
    parser.add_argument(
        "--config",
        metavar="config",
    ...
    other_service = int(args.other_service)
    ...
    try:
        ...
        if mode == "other":
            ...
            print("public mode: {0:b}".format(common.settings.get("public_mode")))
            web.password = None
        if persist:
            common.settings.set("save_private_key", True)
            common.settings.save()
    ...
    t = None
    if mode == "other":
        print("* We assume that you are running your own server locally. *")
    else:
    ...
    try:  # Trap Ctrl-C
        ...
        while mode == "other" or (t != None and t.is_alive()):
            if app.autostop_timer > 0:
